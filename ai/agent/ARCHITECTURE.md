# Agent Memory 综合架构设计

## 一、核心价值分析

### 🏆 最有价值的思路（Top 5）

#### 1. **HindSight - Retain/Recall/Reflect 三操作范式** ⭐⭐⭐⭐⭐
- **核心价值**：将记忆操作抽象为三个基础原语，这是最优雅的设计
- **关键创新**：
  - Retain：结构化存储（而非简单堆砌）
  - Recall：智能检索（而非全量加载）
  - Reflect：推理更新（而非静态存储）
- **适用场景**：所有需要记忆的 Agent 系统

#### 2. **CogMem - 分层记忆架构** ⭐⭐⭐⭐⭐
- **核心价值**：模仿人类认知的三层结构，符合认知科学原理
- **关键创新**：
  - FoA（注意力焦点）：当前工作区
  - DA（直接访问）：快速检索区
  - LTM（长期记忆）：持久存储区
- **适用场景**：长对话、多轮交互系统

#### 3. **A-Mem - Zettelkasten 动态记忆网络** ⭐⭐⭐⭐
- **核心价值**：记忆不是孤立的，而是相互关联的知识网络
- **关键创新**：
  - 动态索引和链接机制
  - 记忆进化（新记忆触发旧记忆更新）
  - 结构化属性（上下文、关键词、标签）
- **适用场景**：知识密集型任务、需要推理的系统

#### 4. **LightMem - 三层记忆 + 睡眠更新机制** ⭐⭐⭐⭐
- **核心价值**：轻量级设计 + 创新的"睡眠时间"更新机制
- **关键创新**：
  - 感觉记忆过滤器（Light1）
  - 短时记忆话题管家（Light2）
  - 长时记忆与睡眠时间更新器（Light3）
- **适用场景**：资源受限场景、需要高效更新的系统

#### 5. **MemMachine - 多类型记忆整合** ⭐⭐⭐⭐
- **核心价值**：不同场景需要不同类型的记忆
- **关键创新**：
  - 情景记忆（事件级）
  - 语义记忆（概念级）
  - 用户画像记忆（个性化）
- **适用场景**：通用 Agent 平台

### 💡 其他有价值的思路

- **ACON**：上下文压缩 + 模型蒸馏（降低计算成本）
- **Every Token Counts**：端到端可学习检索（提升检索质量）
- **O-Mem**：主动用户画像（个性化增强）
- **STMA**：时空记忆整合（具身任务）
- **Titans**：测试时训练（在线学习）

---

## 二、综合架构设计：UniMem（统一记忆系统）

### 设计理念

**"分层存储 + 动态网络 + 智能操作"**

结合认知科学、信息检索、知识图谱的最佳实践，设计一个既符合人类认知规律，又具备工程可行性的记忆系统。

### 架构总览

```
┌─────────────────────────────────────────────────────────┐
│                    UniMem 统一记忆系统                      │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐ │
│  │  操作层      │  │  存储层      │  │  网络层      │ │
│  │ (Operations) │  │  (Storage)   │  │  (Network)   │ │
│  └──────────────┘  └──────────────┘  └──────────────┘ │
│         │                 │                 │            │
│  Retain │ Recall │ Reflect   分层记忆系统    动态记忆网络  │
│         │                 │                 │            │
└─────────┼─────────────────┼─────────────────┼──────────┘
          │                 │                 │
    ┌─────┴─────┐    ┌─────┴─────┐    ┌─────┴─────┐
    │ FoA       │    │ DA        │    │ LTM       │
    │ (工作记忆) │    │ (快速访问) │    │ (长期存储) │
    └───────────┘    └───────────┘    └───────────┘
```

### 核心组件

#### 1. **操作层（Operation Layer）** - 基于 HindSight

```python
class UniMemOperations:
    """
    统一记忆操作接口
    基于 HindSight 的 Retain/Recall/Reflect 范式
    """
    
    def retain(self, experience, context):
        """
        保留：将经验转化为结构化记忆
        - 提取关键信息
        - 生成结构化表示
        - 建立索引和链接
        """
        pass
    
    def recall(self, query, memory_type=None):
        """
        回忆：根据查询检索相关记忆
        - 多模态检索（语义、时间、空间）
        - 分层检索（FoA -> DA -> LTM）
        - 相关性排序
        """
        pass
    
    def reflect(self, memories, current_task):
        """
        反思：基于记忆进行推理和更新
        - 记忆关联分析
        - 信念更新
        - 记忆网络优化
        """
        pass
```

#### 2. **存储层（Storage Layer）** - 基于 CogMem + LightMem

```python
class UniMemStorage:
    """
    分层记忆存储系统
    结合 CogMem 的三层架构和 LightMem 的轻量设计
    """
    
    class FocusOfAttention:
        """
        注意力焦点（FoA）- 工作记忆
        - 容量：小（~100 tokens）
        - 速度：极快（内存访问）
        - 用途：当前任务上下文
        """
        pass
    
    class DirectAccess:
        """
        直接访问（DA）- 快速检索区
        - 容量：中（~10K tokens）
        - 速度：快（缓存访问）
        - 用途：最近相关记忆
        - 更新：LRU + 相关性评分
        """
        pass
    
    class LongTermMemory:
        """
        长期记忆（LTM）- 持久存储
        - 容量：大（无限制）
        - 速度：慢（磁盘/数据库）
        - 用途：所有历史记忆
        - 结构：
          - 情景记忆（事件序列）
          - 语义记忆（概念图谱）
          - 用户画像（个性化信息）
        """
        pass
```

#### 3. **网络层（Network Layer）** - 基于 A-Mem + Graph of Records

```python
class UniMemNetwork:
    """
    动态记忆网络
    基于 A-Mem 的 Zettelkasten 方法和图结构
    """
    
    class MemoryNode:
        """
        记忆节点
        - 内容：结构化表示
        - 属性：上下文、关键词、标签、时间戳
        - 链接：指向相关记忆的边
        """
        pass
    
    class MemoryGraph:
        """
        记忆图谱
        - 节点：记忆单元
        - 边：关联关系（语义、时序、因果等）
        - 动态更新：新记忆触发旧记忆更新
        """
        pass
    
    def add_memory(self, memory):
        """添加记忆并建立链接"""
        pass
    
    def evolve_memory(self, new_memory):
        """记忆进化：新记忆触发网络更新"""
        pass
```

#### 4. **压缩层（Compression Layer）** - 基于 ACON

```python
class UniMemCompression:
    """
    上下文压缩
    基于 ACON 的压缩和蒸馏机制
    """
    
    def compress_context(self, context, target_length):
        """压缩上下文，保留关键信息"""
        pass
    
    def distill_compressor(self):
        """将压缩器蒸馏到小模型"""
        pass
```

#### 5. **检索层（Retrieval Layer）** - 基于 Every Token Counts

```python
class UniMemRetrieval:
    """
    智能检索系统
    基于 Every Token Counts 的端到端可学习检索
    """
    
    def learnable_retrieval(self, query, memory_pool):
        """端到端可学习的检索机制"""
        pass
    
    def hierarchical_retrieval(self, query):
        """分层检索：FoA -> DA -> LTM"""
        pass
```

---

## 三、完整架构实现

### 系统流程

```
用户输入
    ↓
┌─────────────────┐
│  1. Recall      │ ← 从记忆网络中检索相关记忆
│  (回忆)         │
└─────────────────┘
    ↓
┌─────────────────┐
│  2. FoA 更新    │ ← 将检索结果加载到工作记忆
│  (工作记忆)     │
└─────────────────┘
    ↓
┌─────────────────┐
│  3. 推理/执行   │ ← LLM 基于工作记忆进行推理
│  (Agent 核心)   │
└─────────────────┘
    ↓
┌─────────────────┐
│  4. Retain      │ ← 将新经验转化为结构化记忆
│  (保留)         │
└─────────────────┘
    ↓
┌─────────────────┐
│  5. 网络更新    │ ← 添加到记忆网络并建立链接
│  (记忆图谱)     │
└─────────────────┘
    ↓
┌─────────────────┐
│  6. Reflect     │ ← 反思并优化记忆网络
│  (反思)         │
└─────────────────┘
    ↓
┌─────────────────┐
│  7. 分层存储    │ ← 根据重要性存储到不同层级
│  (FoA/DA/LTM)   │
└─────────────────┘
```

### 关键技术点

#### 1. **记忆表示**

```python
@dataclass
class Memory:
    """统一记忆表示"""
    id: str
    content: str  # 原始内容
    embedding: np.ndarray  # 向量表示
    metadata: dict  # 元数据
        - timestamp: 时间戳
        - context: 上下文
        - keywords: 关键词
        - tags: 标签
        - importance: 重要性评分
    links: List[str]  # 关联的记忆ID
    type: str  # 记忆类型（情景/语义/画像）
```

#### 2. **分层存储策略**

```python
class StorageStrategy:
    """
    存储策略：根据记忆特性决定存储位置
    """
    
    def store(self, memory: Memory):
        # 1. 所有记忆先进入 FoA（工作记忆）
        self.foa.add(memory)
        
        # 2. 根据重要性决定是否进入 DA
        if memory.importance > DA_THRESHOLD:
            self.da.add(memory)
        
        # 3. 所有记忆最终进入 LTM
        self.ltm.add(memory)
        
        # 4. 更新记忆网络
        self.network.add_node(memory)
        self.network.create_links(memory)
```

#### 3. **智能检索策略**

```python
class RetrievalStrategy:
    """
    检索策略：分层检索 + 多模态检索
    """
    
    def retrieve(self, query: str, top_k: int = 10):
        results = []
        
        # 1. 从 FoA 检索（最快）
        foa_results = self.foa.search(query, top_k=top_k)
        results.extend(foa_results)
        
        # 2. 从 DA 检索（如果 FoA 结果不足）
        if len(results) < top_k:
            da_results = self.da.search(query, top_k=top_k-len(results))
            results.extend(da_results)
        
        # 3. 从 LTM 检索（如果仍不足）
        if len(results) < top_k:
            ltm_results = self.ltm.search(query, top_k=top_k-len(results))
            results.extend(ltm_results)
        
        # 4. 通过记忆网络扩展（找到相关记忆）
        network_results = self.network.expand(results)
        results.extend(network_results)
        
        # 5. 去重和排序
        return self.rank_and_deduplicate(results)
```

#### 4. **记忆进化机制**

```python
class MemoryEvolution:
    """
    记忆进化：新记忆触发旧记忆更新
    基于 A-Mem 的进化机制
    """
    
    def evolve(self, new_memory: Memory):
        # 1. 找到相关记忆
        related = self.network.find_related(new_memory)
        
        # 2. 更新相关记忆的上下文表示
        for memory in related:
            memory.metadata['context'] = self.update_context(
                memory.metadata['context'],
                new_memory
            )
        
        # 3. 更新记忆网络结构
        self.network.optimize_links(new_memory)
        
        # 4. 触发重要性重新评估
        self.reassess_importance(related)
```

#### 5. **上下文压缩**

```python
class ContextCompression:
    """
    上下文压缩：基于 ACON
    """
    
    def compress(self, memories: List[Memory], max_tokens: int):
        # 1. 重要性排序
        sorted_memories = sorted(memories, key=lambda m: m.importance, reverse=True)
        
        # 2. 压缩低重要性记忆
        compressed = []
        token_count = 0
        
        for memory in sorted_memories:
            if token_count + memory.token_count <= max_tokens:
                compressed.append(memory)
                token_count += memory.token_count
            else:
                # 压缩这个记忆（保留关键信息）
                compressed_memory = self.compress_memory(memory)
                compressed.append(compressed_memory)
                break
        
        return compressed
```

---

## 四、实现优先级

### Phase 1: 核心功能（MVP）
1. ✅ 三层存储架构（FoA/DA/LTM）
2. ✅ Retain/Recall/Reflect 操作
3. ✅ 基础记忆网络（节点+链接）

### Phase 2: 智能增强
4. ✅ 端到端可学习检索
5. ✅ 记忆进化机制
6. ✅ 上下文压缩

### Phase 3: 高级特性
7. ✅ 多类型记忆（情景/语义/画像）
8. ✅ 时空感知记忆
9. ✅ 个性化用户画像

---

## 五、技术栈建议

- **存储**：Redis（FoA/DA）+ PostgreSQL/Vector DB（LTM）
- **向量检索**：Milvus / Qdrant / Pinecone
- **图数据库**：Neo4j（记忆网络）
- **压缩模型**：基于 ACON 的蒸馏模型
- **检索模型**：端到端可学习的检索器

---

## 六、优势总结

1. **认知科学基础**：符合人类记忆的三层结构
2. **工程可行性**：分层设计便于实现和优化
3. **动态进化**：记忆网络持续优化
4. **高效检索**：分层检索 + 智能排序
5. **资源优化**：压缩机制降低计算成本
6. **可扩展性**：模块化设计，易于扩展

---

## 七、下一步行动

1. 实现 MVP 版本（Phase 1）
2. 设计记忆表示格式
3. 实现基础存储和检索
4. 集成到现有 Agent 系统
5. 评估和优化
